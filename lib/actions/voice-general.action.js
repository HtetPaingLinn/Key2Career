...
export async function fixUserInterviewIds(jwtToken) {
  try {
    ...
    console.log("Found Firebase user ID:", firebaseUserId, "for email:", userEmail);

    // Find all interviews where userId contains the user's email (indicating it's from conversation name)
    const interviewsSnapshot = await db.collection("interviews").get();
    const interviewsToUpdate = [];

    interviewsSnapshot.docs.forEach(doc => {
      const data = doc.data();
      // Check if userId contains the email (common pattern from Vapi conversation names)
      if (data.userId && data.userId !== firebaseUserId && 
          (data.userId.includes(userEmail.split('@')[0]) || data.userId.includes(userEmail))) {
        interviewsToUpdate.push({
          id: doc.id,
    ...
    console.log(`Found ${interviewsToUpdate.length} interviews to update:`, interviewsToUpdate);

    // Update all interviews
    const updatePromises = interviewsToUpdate.map(interview => 
      db.collection("interviews").doc(interview.id).update({ userId: firebaseUserId })
    );

    await Promise.all(updatePromises);

    console.log(`Successfully updated ${interviewsToUpdate.length} interviews with correct userId:`, firebaseUserId);
    return { success: true, updatedCount: interviewsToUpdate.length };
  } catch (error) {
