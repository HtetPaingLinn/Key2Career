    ...
    <div className="w-full h-full flex bg-gray-100 print:bg-white text-xs font-sans relative">
      ...
      <aside className="w-1/3 min-w-[280px] bg-emerald-900 text-white flex flex-col p-5 print:bg-emerald-900 rounded-l-2xl shadow-lg">
        ...
        {cvData.certifications && cvData.certifications.length > 0 && (
          <div className="mb-5">
            <h3 className="uppercase font-bold text-emerald-100 text-[11px] mb-2 tracking-widest flex items-center gap-2">
              <FontAwesomeIcon icon={faCertificate} className="w-3 h-3" /> Certifications
            </h3>
            <ul>
              {cvData.certifications.map((cert, i) => (
                <li key={i} className="mb-0.5">
                  <div className="font-semibold text-xs">{cert.name}</div>
                  <div className="text-emerald-200 text-[10px]">{cert.issuingOrganization} {cert.date && `(${formatDate(cert.date)})`}</div>
                </li>
              ))}
            </ul>
          </div>
