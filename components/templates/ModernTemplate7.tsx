...
export function ModernTemplate7({ cvData }) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto bg-white shadow-sm border border-gray-200">
      {/* Header with Accent */}
      <div className="bg-white border-b-4 border-red-600 p-8">
        <div className="flex items-center">
          ...
          <div className="flex-1">
            <h1 className="text-3xl font-normal text-gray-900 mb-2 break-words">{data.personalInfo.name}</h1>
            <p className="text-red-700 text-lg break-words">{data.personalInfo.title}</p>
          </div>
          <div className="text-right text-sm text-gray-600 space-y-1">
            ...
            {data.personalInfo.phone && (
              <div className="break-all">{data.personalInfo.phone}</div>
            )}
            {data.personalInfo.location && (
              <div className="break-words">{data.personalInfo.location}</div>
            )}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {data.technicalSkills.map((skill, index) => (
                    <div key={index} className="text-sm text-gray-700 py-1 border-l-2 border-red-600 pl-3 break-words">
                      {skill}
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-2">
                  {data.awardsAndHonors.map((award, index) => (
                    <div key={index} className="text-sm text-gray-700 leading-relaxed">
                      ...
                      <div className="text-red-700 text-xs break-words">{award.issuer}</div>
                      {award.year && (
                ...
                <div>
                  ...
                  {data.projects.map((project, index) => (
                    <div key={index} className="mb-4">
                      ...
                      {project.tech && (
                        <p className="text-red-700 text-xs break-words">{project.tech}</p>
                      )}
                      {project.description && (
