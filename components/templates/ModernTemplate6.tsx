...
export function ModernTemplate6({ cvData }) {
    ...
    <div className="w-full max-w-4xl mx-auto bg-white shadow-sm border border-gray-200">
      ...
      <div className="bg-slate-800 p-8">
        <div className="flex items-center text-white">
          <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center mr-8 border-2 border-cyan-400">
          ...
          <div className="flex-1">
            ...
            <p className="text-cyan-200 text-lg break-words">{data.personalInfo.title}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-50 px-8 py-4 border-b border-gray-200">
        <div className="flex justify-center space-x-8 text-sm text-gray-600">
          ...
          {data.personalInfo.website && (
            <div className="flex items-center">
              <Globe size={14} className="mr-2 text-cyan-600" />
              ...
              <div>
                ...
                <div className="space-y-2">
                  {data.technicalSkills.slice(0, 8).map((skill, index) => (
                    <div key={index} className="text-sm text-gray-700 py-1 border-l-2 border-cyan-600 pl-3 break-words">
                      {skill}
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-2">
                  {data.softSkills.map((skill, index) => (
                    <div key={index} className="flex items-center text-sm text-gray-700">
                      <div className="w-1.5 h-1.5 bg-cyan-600 rounded-full mr-3 flex-shrink-0"></div>
          ...
          <div className="col-span-3 space-y-8">
                  ...
                  <div key={index} className="mb-6">
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex-1">
                        ...
                        <p className="text-cyan-700 font-medium break-words">{job.company}</p>
                        {job.location && (
            ...
            <div className="grid grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  {data.education.map((edu, index) => (
                    <div key={index} className="mb-4">
                      ...
                      <p className="text-cyan-700 text-sm break-words">{edu.school}</p>
                      {edu.field && (
                        <p className="text-gray-600 text-xs break-words">{edu.field}</p>
                      )}
                      <p className="text-gray-500 text-xs break-words">{edu.period}</p>
                      {edu.description && (
                        <p className="text-gray-600 text-xs leading-relaxed mt-1 break-words">
                          {edu.description}
                        </p>
                      )}
                    </div>
                ...
                <div>
                  ...
                  {data.projects.map((project, index) => (
                    <div key={index} className="mb-4">
                      ...
                      {project.tech && (
                        <p className="text-cyan-700 text-xs break-words">{project.tech}</p>
                      )}
                      {project.description && (
              ...
              {data.awardsAndHonors.length > 0 && (
                <div>
                  <h3 className="text-lg font-medium text-gray-900 mb-4">Awards & Honors</h3>
                  {data.awardsAndHonors.map((award, index) => (
                    <div key={index} className="mb-2 text-sm text-gray-700">
                      ...
                      <div className="text-cyan-700 text-xs break-words">{award.issuer}</div>
                      {award.year && (
