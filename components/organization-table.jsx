...
function RowActions({ row }) {
  ...
  const canConfirm =
    ...
    (openAction === "approve" || banReason.trim() !== "");
...
export function OrganizationTable({ initialData = [] }) {
  ...
  const waitingForApprovalCount = React.useMemo(() => {
    return data.filter(
      (item) => normalizeStatus(item.status) === "waiting_for_approval"
    ).length;
  }, [data]);
  ...
  return (
    <>
      ...
      <Tabs
        ...
        className="w-full flex-col justify-start gap-6 mt-6 mb-12"
      >
        ...
        >
          ...
          <div className="flex items-center justify-between px-4">
            <div className="text-muted-foreground hidden flex-1 text-sm lg:flex">
              {waitingForApprovalCount} of{" "}
              {table.getFilteredRowModel().rows.length} organization(s) waiting
              for approval.
            </div>
