...
export default function VolunteerSection({ data, onChange, onAdd, onRemove }) {
  // State for AI/grammar per volunteer entry
          ...
          <div key={idx} className="border border-gray-200 rounded-xl p-6 mb-4 relative bg-white shadow-sm">
            ...
            <div className="mt-4">
              ...
              <div className="flex gap-2 mt-2">
                ...
                <button type="button" className="px-2 py-1 text-xs rounded bg-blue-100 text-blue-700 border border-blue-200 hover:bg-blue-200" onClick={() => handleGrammarCheck(idx, vol.description)} disabled={grammarLoading}>📝 Check Grammar</button>
              </div>
              {aiPromptIdx === idx && (
                <div className="mt-2 flex flex-col gap-2">
                  <input type="text" className="w-full rounded border border-gray-300 px-2 py-1 text-xs" placeholder="Describe what you want to generate (e.g. 'Volunteer summary')" value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} />
                  <button type="button" className="px-2 py-1 text-xs rounded bg-emerald-600 text-white hover:bg-emerald-700" onClick={() => handleGenerate(idx, aiPrompt, vol.description)} disabled={aiLoading || !aiPrompt}>{aiLoading ? 'Generating...' : 'Generate'}</button>
